...
/* Found at:
 * https://github.com/Opendigitalradio/ka9q-fec/blob/master/decode_rs_ccsds.c
 */
#ifdef DEBUG
#include <stdio.h>
#endif
...
int decode_rs_ccsds(data_t *data,int *eras_pos,int no_eras,int pad) {
  ...
  for(i=0;i<NN-pad;i++)
    cdata[i] = TalToConventional[data[i]];

  r = _decode_rs_8(cdata,eras_pos,no_eras,pad);

  if(r > 0){
    /* Convert from conventional to dual basis */
    for(i=0;i<NN-pad;i++)
      data[i] = TalToDualBasis[cdata[i]];
  }
  return r;
}
